  ...
  while(omx_base_component_Private->state == OMX_StateIdle || 
    ...
    if(pInputSem->semval>0 && isInputBufferNeeded==OMX_TRUE ) {
      ...
      if(pInputQueue->nelem>0){
        ...
        pInputBuffer = omx_dequeue(pInputQueue);
    ...
    if(isInputBufferNeeded==OMX_FALSE) {
      ...
      if (omx_base_sink_Private->BufferMgmtCallback && pInputBuffer->nFilledLen > 0) {
        ...
        DEBUG(DEB_LEV_FULL_SEQ, "If no buffer management call back, explicitly consume input buffer. nInputPortIndex=%d   BufferMgmtCallback=0x%x  nFilledLen=%d\n",
          pInputBuffer->nInputPortIndex, omx_base_sink_Private->BufferMgmtCallback, pInputBuffer->nFilledLen);
